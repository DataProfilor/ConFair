...
def assign_pred_mcc_weight_group(x, vio_means, n_vio=4): # assign prediction based on weighted group violation
    weights = []
    for i, mean_i in enumerate(vio_means):
        if mean_i > 0.1:
            weights.append(1-x.iloc[i])
    ...
    if weights[0] > weights[1]: # use negative probability for group 0
        weight_g0 = weights[0]
    ...
    if setting == 'multi':
        ...
        for name_suffix, cur_pred_col in zip(['', '-G0', '-G1'], ['Y_pred', 'Y_pred_G0', 'Y_pred_G1']):
            ...
            if sum(test_predict) == 0:
                print('==> model predict only one label for test data in DifFair ', cur_pred_col, data_name, model_name, seed)
            test_df[cur_pred_col] = test_predict
            val_predict = generate_model_predictions(opt_model, val_data)
            if sum(val_predict) == 0:
                print('==> model predict only one label for val data in DifFair ', cur_pred_col, data_name, model_name, seed)
